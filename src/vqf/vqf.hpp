...
#define VQF_SINGLE_PRECISION // AlfredoVQF: enabled for MCU performance (only change from upstream; Butterworth filters stay double per upstream design)
...
public:
     ...
     *
     *     VQF vqf(0.01); // 0.01 s sampling time, i.e. 100 Hz
     *     vqf.updateBatch(gyr, acc, mag, N, nullptr, quat9D, nullptr, nullptr, nullptr, nullptr, nullptr);
     * \endrst
     *
    ...
    /**
     * @brief Performs 3x3 matrix multiplication (\f$\mathbf{M}_\mathrm{out} = \mathbf{M}_1\mathbf{M}_2\f$).
     ...
     * @param out output 3x3 matrix \f$\mathbf{M}_\mathrm{out}\f$ (stored in row-major order)
     */
    ...
    /**
     * @brief Calculates the inverse of a 3x3 matrix (\f$\mathbf{M}_\mathrm{out} = \mathbf{M}^{-1}\f$).
